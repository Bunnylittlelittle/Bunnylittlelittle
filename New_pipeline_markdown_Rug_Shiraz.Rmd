---
title: "DiRT_With_CDS_two_types_statistic_for_One/no_intron"
output: html_document
date: "2023-03-20"
Author: Charlotte Sai
...
gene.info.from.gff <- read_delim("D:/Vitis/Reference_genome/Shiraz_Ruggeri/Ruggeri_reference_genome_fasta/140R.P_contigs.gtf", delim = "\t", col_names = FALSE) %>% 
...
CDS.info.sorted$gene.id <- str_extract(CDS.info.sorted$gene.id, '(?<=gene_id ")[^"]+')
...
CDS.start.end.for.gene <- left_join(CDS.start.point, CDS.end.point, by = "gene.id")%>% dplyr::select("gene.chr.x", "gene.start", "gene.end", "gene.id", "gene.strand.x", "gene.biotype.x") %>%
  set_colnames(c("gene.chr", "gene.start", "gene.end", "gene.id", "gene.strand", "gene.biotype")) %>%
  arrange(gene.chr, gene.end)
##use replaceSeqID codes in draft_codes.Rmd to replace sequence IDs into contig IDs

# output the gene info for bedtools use
write.table(CDS.start.end.for.gene,
           file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_CDS.infor.txt",
           quote = FALSE, sep = "\t",
           row.names = FALSE, col.names = FALSE)

# tRNA info from tRNAscan-SE
tRNA.info <- read_delim("C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_RNA.bed", delim = "\t", col_names = FALSE) %>%
...
write.table(tRNA.info,
           file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_tRNAs_sorted.txt",
           quote = FALSE, sep = "\t",
           row.names = FALSE, col.names = FALSE)
```
...
```{r}
seq.size <- BamFile("D:/Vitis/mix_RNA/Trimmed/mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam") %>%  ##load bam file (aligned RNA seq file) from your directory
  seqinfo() %>% 
  as.data.frame() %>% 
...
write.table(seq.size,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_genome.txt",
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = FALSE)
```
...
bedtools sort -i Rug_CDS.infor.txt > Rug_CDS.infor.sorted.txt
bedtools closest -id -a Rug_tRNAs_sorted.txt -b Rug_CDS.infor.sorted.txt -g Rug_genome.txt -D ref > Rug_closest_tRNA_gene_upS.bed
bedtools closest -iu -a Rug_tRNAs_sorted.txt -b Rug_CDS.infor.sorted.txt -g Rug_genome.txt -D ref > Rug_closest_tRNA_gene_downS.bed

## combine two result files from BedTools and sort, then refine info into proper format
```{r}
closest_tRNA_CDS.up <- read.delim("C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_closest_tRNA_gene_upS.bed", header=FALSE)
closest_tRNA_CDS.down <- read.delim("C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_closest_tRNA_gene_downS.bed", header=FALSE)
...
# write.table(tRNA.CDS.combo.tRNA.UTR.overlap,
#             file = "C:/AAAStudy/PhD/Bioinformatics/Phylogenetic/Vitis_vinifera/Result/tRNA_in_gene_UTR.txt",
#             quote = FALSE, sep = "\t",
#             row.names = FALSE, col.names = FALSE)
```
...
```{r}
Seqinfo <- read_delim("C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/Rug_genome.txt", 
...
```{r}
# specify aligned RNA seq Samples
bam.dir <- "D:/Vitis/mix_RNA/Trimmed/" # Directory for bamfiles pay attention this is the directory not the bam files themselves
...
counts <- lapply (bamfiles,function(bamfile){
  ...
  countOverlaps(GRange_tRNA, bam.input,
                maxgap=-1L, minoverlap=1L,  ###Fei:this can be set, Charlotte set this as 1 bp overlap (minoverlap=1L).
                type=c("any"),
                select=c("all"),
                ignore.strand=TRUE)
})
...
# pull out the exon and UTR information from gff
#get exon info
exon.infor <- gene.info.from.gff %>%
  filter(gene.biotype == "exon") %>% # keep the type of annotation that you are interested to explore
  arrange(gene.chr, gene.start) 
exon.infor$gene.id <- str_extract(exon.infor$gene.id, '(?<=gene_id ")[^"]+')
exon.infor <- exon.infor %>% set_colnames(c("chr", "start", "end","gene.id", "gene.strand", "exon_UTR_id"))
#assign numbers for each exon
exon.num <- sapply(as.character(unique(exon.infor$gene.id)), function(g){
  rowNb <- which(g == exon.infor$gene.id)
  strand <- exon.infor$gene.strand[rowNb]
  start <- exon.infor$start[rowNb]
  order(ifelse(strand =="+", start, -start))
}) %>% unlist()
exon.infor %<>% mutate(`exon_UTR_id` = paste0(`exon_UTR_id`,".",exon.num))
#===============================================================================#
start5.infor <- gene.info.from.gff %>%
  filter(gene.biotype == "start_codon") %>% # keep the type of annotation that you are interested to explore
  arrange(gene.chr, gene.start) 
start5.infor$gene.id <- str_extract(start5.infor$gene.id, '(?<=gene_id ")[^"]+') 
start5.infor <- start5.infor %>% set_colnames(c("chr","start","end","gene.id","gene.strand","exon_UTR_id")) %>%
 filter(gene.id %in% input.data$gene.id)  %>%
  na.omit() %>%
  arrange(chr,start)
# give unique numbers to UTRs that are belongs to same genes
UTR.num <- sapply(as.character(unique(start5.infor$gene.id)), function(g){
  rowNb <- which(g == start5.infor$gene.id)
  strand <- start5.infor$gene.strand[rowNb]
  start <- start5.infor$start[rowNb]
  order(ifelse(strand =="+", start, -start))
}) %>% unlist()
start5.infor %<>% mutate(`exon_UTR_id` = paste0(`exon_UTR_id`,".",UTR.num))
#===============================================================================#
stop3.infor <- gene.info.from.gff %>%
  filter(gene.biotype == "stop_codon") %>% # keep the type of annotation that you are interested to explore
  arrange(gene.chr, gene.start) 
stop3.infor$gene.id <- str_extract(stop3.infor$gene.id, '(?<=gene_id ")[^"]+') 
stop3.infor <- stop3.infor %>% set_colnames(c("chr","start","end","gene.id","gene.strand","exon_UTR_id")) %>%
 filter(gene.id %in% input.data$gene.id) %>%
  na.omit() %>%
  arrange(chr,start)
# give unique numbers to UTRs that are belongs to same genes
UTR.num <- sapply(as.character(unique(stop3.infor$gene.id)), function(g){
  rowNb <- which(g == stop3.infor$gene.id)
  strand <- stop3.infor$gene.strand[rowNb]
  start <- stop3.infor$start[rowNb]
  order(ifelse(strand =="+", start, -start))
}) %>% unlist()
stop3.infor %<>% mutate(`exon_UTR_id` = paste0(`exon_UTR_id`,".",UTR.num))
# bind them all together and sorted
UTR_exon.info <- rbind(exon.infor,
                       start5.infor,
                       stop3.infor) %>%
   mutate(`exon_UTR_id` = paste0(`gene.id`,".",`exon_UTR_id`)) %>%
...
tgene.expressed.exon <- left_join(tRNA.CDS.expressed.combo,exon.infor, by = "gene.id") 
tgene.expressed.exon2 <- tgene.expressed.exon  %>% filter(`exon_UTR_id` == "exon.2")
tgene.only.one.exon <- subset(tgene.expressed.exon , !(id %in% tgene.expressed.exon2$id))
#25 candidates with no intron in Ruggerie tRNA gene expressed

write.table(tgene.only.one.exon,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/Mix_Rug_tGene_expressed_one_exon.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)

### filter positive result with only 2 exon (one intron)

tgene.expressed.exon3 <- tgene.expressed.exon %>% filter(`exon_UTR_id` == "exon.3")
tgene.expressed.with1and2exon <- subset(tgene.expressed.exon, !(id %in% tgene.expressed.exon3$id)) 
tgene.expressed.with2exon <-  subset(tgene.expressed.with1and2exon, !(id %in% tgene.only.one.exon$id))  %>% filter(`exon_UTR_id` == "exon.2")

write.table(tgene.expressed.with2exon,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/Mix_Rug_tGene_expressed_two_exon.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
...
DicistronicName <- names(coverage.intergenic$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) 
...
IntronName <- names(coverage.intron.closest$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
IntronName <- names(coverage.intron.2nd.closest$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
```{r}
allIntergenic <- names(coverage.intergenic$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # pull out sample name from any sample
...
# export results
```{r}
continuous.coverage.final.all <- continuous.coverage.final%>%
...
gene.info.sorted <- gene.information %>%
  arrange(gene.chr, gene.start) 
...
Dicistronic.tRNA.gene.coordinate <- left_join(Dicistronic.all.tRNA, Dicistronic.gene, by = "gene.id") %>%
  ...
  set_colnames(c("id", "chr", "start", "end", "gene.id", "gene.start", "gene.end", "gene.strand", "tRNA.id", "tRNA.start", "tRNA.end", "tRNA.strand", "tRNA.upstream", "tRNAFirst", "interval.length", "continu.cov", "combined.continu.cov")) %>%  mutate(tRNA.overlap.UTR = ifelse(tRNAFirst == 'TRUE' & gene.start < tRNA.end, TRUE,
   ...
   mutate(start = ifelse(tRNAFirst, tRNA.start, gene.start),
         end = ifelse(tRNAFirst, gene.end, tRNA.end)) %>%
...
write.table(tRNA_in_UTR_all,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/Mix_tRNA_in_UTR_all.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
write.table(Dicistronic.tRNA.gene.coordinate,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/Mix_Final_Result.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
IntronName <- names(coverage.intron.one$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
trim()
```
## use ``Cal_Coverage()`` to look through ``bam`` files for coverage of intron mimic
...
IntronName <- names(coverage.One.intron.mimic$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
write.table(continuous.coverage.final.one.intron,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/Mix_Rug_One_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
tgene.expressed.no.intron <- left_join(tgene.only.one.exon,tRNA.CDS.combo,by = "gene.id") %>%
  dplyr::select(chr.x, gene.start.x, gene.end.x, gene.id, gene.strand.x) %>%
...
write.table(tgene.expressed.no.intron,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/mix_Rug_tRNA_gene_expressed_no_intron.txt",
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = FALSE)
...
IntronName <- names(coverage.no.intron.mimic.intron1$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
IntronName <- names(coverage.no.intron.mimic.intron2$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
gene.id.with.two.intron <- exon.infor %>%
  filter(exon_UTR_id== "exon.3")
gene.with.two.intron.info <- merge(gene.id.with.two.intron, CDS.start.end.for.gene, by = "gene.id") %>%
  dplyr::select(chr, gene.start, gene.end, gene.id, gene.strand.y, gene.biotype ) %>%
    arrange(chr, gene.start) 
write.table(gene.with.two.intron.info,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/mix_Rug_Nearby_gene_with_two_intron.txt",
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = FALSE)
```
...
module load BEDTools
bedtools closest -io -a mix_Rug_tRNA_gene_expressed_no_intron.txt -b mix_Rug_Nearby_gene_with_two_intron.txt -g Rug_genome.txt -D ref > mix_Rug_closest_gene_gene.bed


```{r}
#combine gene information without intron to nearby gene information with two introns.
closest_gene.no.intron_gene <- read.delim("C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Essentail_file/mix_Rug_closest_gene_gene.bed", header=FALSE) %>% 
  unique() %>%
...
#=========================================================================#
#For some gff files, there are intron annotation in there. However, I want to create it by myself with Charlotte's method, because there are differences between intron information from annotation files and intron information from Charlotte's method. Therefore, to be consist with the information above, I recommend to use Charlotte's method. 
# 
...
# It looks like the first or last exon embl plants defined acturaly take UTR into a count as a part of it. if UTR info is provided, the intron in the UTR region can be found. otherwise, it returns intron info in the following code as typical intron, which is between two exon is a intron.
# pull out intron coordinates based on the data above
UTR_exon.all.gene.set1 <- exon.infor %>% 
  dplyr::select(chr,start,end,exon_UTR_id,gene.id, gene.strand) %>%
  set_colnames(c("chr","start.set1","end.set1",
                 "exon_UTR_id.set1","gene.id.set1","gene.strand.set1"))

UTR_exon.all.gene.set2 <- exon.infor[-1,] %>% 
  dplyr::select(start,end,exon_UTR_id,gene.id, gene.strand) %>%
  set_colnames(c("start.set2","end.set2",
                 "exon_UTR_id.set2","gene.id.set2","gene.strand.set2")) %>%
  rbind(c(0,0,NA,NA))
...
IntronName <- names(coverage.nearby.intron1$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
IntronName <- names(coverage.nearby.intron2$mix_EE1_MG2_0.1_Rug_hisat2_sorted.bam) # same as DicistronicName
...
write.table(continuous.coverage.final.no.intron,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/mix_Rug_Result_with_no_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
Dicistronic.UTR5.info <- start5.infor %>% filter(gene.id %in% Dicistronic.all.tRNA$gene.id) 
Dicistronic.UTR3.info <- stop3.infor %>% filter(gene.id %in% Dicistronic.all.tRNA$gene.id) 
#Remember that until now the gene start and end in dicistronic information are CDS start end coordinate, not the real gene start/end coordinate. I will make real gene coordinate underneath.
...
gene.info.sorted <- gene.information  %>% # keep the type of annotation that you are interested to explore
  arrange(gene.chr, gene.start) 
Dicistronic.gene <- gene.info.sorted %>% filter(gene.id %in% continuous.coverage.final.all$gene.id) %>% unique()
...
Dicistronic.tRNA.gene.coordinate <- left_join(Dicistronic.all.tRNA, Dicistronic.gene, by = "gene.id") %>%
  ...
  dplyr::select(id, chr, start, end, gene.id, gene.start, gene.end, gene.strand, tRNA.id, tRNA.start, tRNA.end, tRNA.strand, tRNA.upstream, tRNAFirst, interval.length, continu.cov, combined.continu.cov, tRNA.overlap.UTR)
  tRNA.UTR.final <- table(Dicistronic.tRNA.gene.coordinate$tRNA.overlap.UTR == "TRUE")
#get a table show how many candidates have tRNA overlap utr how many not
tRNA_in_UTR_all <- data.frame(
  "tRNA.UTR.tgene.combo" = tRNA.UTR.tgene.combo,
  "tRNA.UTR.tgene.expressed.combo" = tRNA.UTR.tgene.expressed.combo,
  "tRNA.UTR.final" = tRNA.UTR.final
...
```{r}
write.table(tRNA_in_UTR_all,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/mix_Rug_tRNA_in_UTR_all.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
write.table(Dicistronic.tRNA.gene.coordinate,
            file = "C:/AAAStudy/PhD/Bioinformatics/Ruggeri/Result/mix_Rug_Final_Result.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
