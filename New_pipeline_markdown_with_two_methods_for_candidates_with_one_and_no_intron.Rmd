...
#gene introns from nearby gene, as we just want to use the gene mimic intron 1and 2, we do not need the intron in real order, so I filtered out the duplication intron from multiple transcripts of same one nearby gene.
...
intergeic.sign.final.intron.nearby <- tTestResults.final.intron.nearby$IntergenicVsintron_one_nearby %>% 
  filter(FDR < 0.05, V < 0) 
intron.not.sign.final.intron.nearby <- tTestResults.final.intron.nearby$intron_two_nearbyVsintron_one_nearby %>% 
  filter((FDR < 0.05 & V > 0) | FDR >=0.05)
