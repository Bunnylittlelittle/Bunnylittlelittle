...
gene.info.from.gff <- read_delim("C:/", delim = "\t", col_names = FALSE) %>% 
...
##if gene.info.from.gff has no UTR, then run gene.information, Use ctrl+shift+c to deactive or active the chunk
gene.information <- gene.info.from.gff %>%
  filter(gene.biotype == "gene") %>%
  mutate(gene.id = sub(".*ID=", "",gene.id)) %>%
  arrange(gene.chr, gene.end)

# use mutate and replace to create a new column with the replacements
gene.information <- gene.information %>% 
             mutate(new.gene.chr = replace(gene.chr, 
                                           gene.chr %in% names(replacements), 
                                           replacements[match(gene.chr, names(replacements))])) 
gene.information[,1] <- gene.information[,7]
gene.information <- gene.information[, -7]
...
write.table(gene.information,
           file = "C:/gene.infor.txt",
           quote = FALSE, sep = "\t",
           row.names = FALSE, col.names = FALSE)

# tRNA info from GtRNA database
tRNA.info <- read_delim("C:/tRNA.bed", delim = "\t", col_names = FALSE) %>%
...
```{r}
seq.size <- BamFile("F:/SRR12376435_.sorted.bam") %>%  ##load bam file (aligned RNA seq file) from your directory
  seqinfo() %>% 
  as.data.frame() %>% 
...
bedtools sort -i gene.infor.txt > gene.infor.sorted.txt
bedtools closest -id -a tRNA_sorted.txt -b gene.infor.sorted.txt -g genome.txt -D ref > closest_tRNA_gene_upS.bed
bedtools closest -iu -a tRNA_sorted.txt -b gene.infor.sorted.txt -g genome.txt -D ref > closest_tRNA_gene_downS.bed

## combine two result files from BedTools and sort, then refine info into proper format
```{r}
closest_tRNA_CDS.up <- read.delim("C:/closest_tRNA_gene_upS.bed", header=FALSE)
closest_tRNA_CDS.down <- read.delim("C:/closest_tRNA_gene_downS.bed", header=FALSE)
...
```{r}
Seqinfo <- read_delim("C:/genome.txt", 
...
bam.dir <- "F:/RNAseq/PRJNA650527/" # Directory for bamfiles pay attention this is the directory not the bam files themselves
...
tRNA.expressed.AllSample <- rowSums(dplyr::select(tRNA.counts,6:8) >= 1) == 3 
...
m <- useMart("plants_mart", dataset="atrichopoda_eg_gene", host="plants.ensembl.org")
...
exon.info <- getBM(attributes=c("chromosome_name", "exon_chrom_start", "exon_chrom_end", 
                                "ensembl_exon_id","ensembl_peptide_id", "strand"), mart=m) %>%
...
tgene.expressed.exon <- exon.info %>% separate(col ="exon_UTR_id", sep = "-", into=c("transcript.id","exon_UTR_id")) %>% mutate("exon_UTR_id" = paste0('exon', exon_UTR_id))
...
#             file = "C:/tGene_expressed_no_intron.xlsx",
...
#             file = "C:/tGene_expressed_one_intron.xlsx",
#             quote = FALSE, sep = "\t", row.names = FALSE)
```
...
UTR5.gene.info <- getBM(attributes=c("chromosome_name", "5_utr_start", "5_utr_end", 
                                "ensembl_transcript_id","ensembl_peptide_id", "strand", "start_position", "end_position"), mart=m)

UTR5.info <- getBM(attributes=c("chromosome_name", "5_utr_start", "5_utr_end", 
                                "ensembl_transcript_id","ensembl_peptide_id", "strand"), mart=m)%>%
  set_colnames(c("chr","start","end","exon_UTR_id","gene.id","gene.strand")) %>%
  filter(gene.id %in% input.data$gene.id) %>%
  mutate(#chr = paste0('chr', chr),
         gene.strand = ifelse(gene.strand == 1,"+","-"),
...
DicistronicName <- names(coverage.intergenic$ERR419Sample13.sorted.merged.bam) 
...
IntronName <- names(coverage.intron.closest$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
IntronName <- names(coverage.intron.2nd.closest$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
```{r}
allIntergenic <- names(coverage.intergenic$ERR419Sample13.sorted.merged.bam) # pull out sample name from any sample
...
write.table(continuous.coverage.final,
            file = "C:/Result_with_two_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
IntronName <- names(coverage.intron.one$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
IntronName <- names(coverage.One.intron.mimic$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
write.table(continuous.coverage.final.one.intron,
            file = "C:/Result_with_one_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
write.table(tgene.expressed.no.intron,
            file = "C:/tRNA_gene_expressed_no_intron.txt",
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = FALSE)
...
no.intron.mimic.intron1 <-  do.call("rbind", list(intron1.mimic.for.stats.down,intron1.mimic.for.stats.up)) %>%
  arrange(gene.id) %>% 
  unique()%>% group_by(gene.id) %>% slice_min(n = 1, length)%>%
    ungroup()

no.intron.mimic.intron2 <- do.call("rbind",list(intron2.mimic.for.stats.down,intron2.mimic.for.stats.up)) %>%  
  arrange(gene.id) %>% 
  unique()%>% group_by(gene.id) %>% slice_min(n = 1, length)%>%
    ungroup()
...
IntronName <- names(coverage.no.intron.mimic.intron1$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
IntronName <- names(coverage.no.intron.mimic.intron2$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
exon.info.all.genes %<>% separate(col ="exon_UTR_id", sep = "-", into=c("transcript.id","exon_UTR_id")) %>% mutate("exon_UTR_id" = paste0('exon', exon_UTR_id))
exon.info.all.genes <- exon.info.all.genes[-c(4)]
...
write.table(gene.with.two.intron.info,
            file = "C:/Nearby_gene_with_two_intron.txt",
            quote = FALSE, sep = "\t",
            row.names = FALSE, col.names = FALSE)
```
### Use tRNA_gene_expressed_no_intron.txt and Ruggerie_whole_genome_detailed_sorted.txt 

do this on HPC with bedtool closest ignor overlaping setting

module load BEDTools
bedtools closest -io -a tRNA_gene_expressed_no_intron.txt -b Nearby_gene_with_two_intron.txt -g genome.txt -D ref > closest_gene_gene.bed

```{r}
#combine gene information without intron to nearby gene information with two introns.
closest_gene.no.intron_gene <- read.delim("C:/closest_gene_gene.bed", header=FALSE) %>% 
  unique() %>%
...
IntronName <- names(coverage.nearby.intron1$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
IntronName <- names(coverage.nearby.intron2$ERR419Sample13.sorted.merged.bam) # same as DicistronicName
...
write.table(continuous.coverage.final.no.intron,
            file = "C:/Result_with_no_intron.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
...
gene.info <- getBM(attributes=c("chromosome_name", "start_position", "end_position",
                                "ensembl_peptide_id", "strand", "gene_biotype"), mart=m)  %>%
  set_colnames(c("gene.chr", "gene.start", "gene.end", "gene.id", "gene.strand", "gene.biotype")) %>%
  mutate(#gene.chr = paste0('chr', gene.chr),
         gene.strand = ifelse(gene.strand == 1,"+","-")) %>%  # make it consistant with other data for strand symbol and Chr names, if the format alread matched, this line of code needs to set as comment 
  arrange(gene.chr, gene.start)
...
write.table(tRNA_in_UTR_all,
            file = "C:/tRNA_in_UTR_all.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
write.table(Dicistronic.tRNA.gene.coordinate,
            file = "C:/Final_Result.xlsx",
            quote = FALSE, sep = "\t", row.names = FALSE)
```
